"""Real-Postgres fixtures for test_workflow_integration.py — the one test
module in this package that needs a real WorkflowExecutor (which persists
via SQLAlchemy), not just in-memory CapabilityRegistry/CapabilityExecutor
objects. Mirrors services/workflow_engine/tests/conftest.py exactly, same
reasoning: a real container, not SQLite (docs/engineering/09-testing-strategy.md
§3), and Base.metadata.create_all() rather than Alembic migrations, since
these tests exercise the capability/workflow bridge, not migration
correctness.
"""
...
async def seeded_project(db: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    ...
    user = User(
        email=f"capreg-{unique}@example.com",
        username=f"capreg{unique}",
        hashed_password="x",
    )
    db.add(user)
    await db.flush()
