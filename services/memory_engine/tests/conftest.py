...
async def seeded_project(db: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    """A minimal User -> Organization -> Project chain for
    architecture_decisions/memory_events to FK against. Returns
    (user_id, org_id, project_id)."""
    unique = uuid.uuid4().hex[:8]
    user = User(
        email=f"memory-{unique}@example.com",
        username=f"mem{unique}",
        hashed_password="x",
    )
    db.add(user)
    await db.flush()
