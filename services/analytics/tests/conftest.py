...
async def _make_project(db: AsyncSession, *, label: str) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    unique = uuid.uuid4().hex[:8]
    user = User(
        email=f"analytics-{label}-{unique}@example.com",
        username=f"an{label}{unique}",
        hashed_password="x",
    )
    db.add(user)
    await db.flush()

    org = Organization(name=f"Test Org {label}", slug=f"org-{label}-{unique}", created_by=user.id)
    db.add(org)
    await db.flush()

    project = Project(
        organization_id=org.id,
        name=f"Test Project {label}",
        slug=f"proj-{label}-{unique}",
        owner_id=user.id,
        created_by=user.id,
    )
    db.add(project)
...
async def seeded_project(db: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    """A minimal User -> Organization -> Project chain. Returns (user_id, org_id, project_id)."""
    return await _make_project(db, label="a")


@pytest.fixture
async def other_seeded_project(db: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    """A second, unrelated User -> Organization -> Project chain — for tenant
    isolation tests that must prove data seeded under `seeded_project` never
    leaks into a metric computed for this one."""
    return await _make_project(db, label="b")
