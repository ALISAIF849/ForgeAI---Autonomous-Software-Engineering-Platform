...
SQLite — same reasoning as apps/api/tests/conftest.py
(docs/engineering/09-testing-strategy.md §3). Uses Base.metadata.create_all()
directly rather than running Alembic migrations: these tests exercise the
Executor's behavior against a real schema, not migration correctness (that's
apps/api/tests/test_migrations.py's job) — importing apps/api's alembic
config from here would be a backwards dependency, a lower-level package's
tests depending on the top-level app.
"""
...
async def _clean_tables(database_url: str) -> None:
    ...
    starts against empty tables. Necessary because `db` opens a fresh engine
    per test but all of them point at the same session-scoped Postgres
    container: without this, e.g. WorkflowQueue's "oldest queued entry first"
    ordering assertions get polluted by rows an earlier test left behind (a
    real failure mode this caught, not a hypothetical one)."""
...
async def seeded_project(db: AsyncSession) -> tuple[uuid.UUID, uuid.UUID, uuid.UUID]:
    ...
    user = User(
        email=f"executor-{unique}@example.com",
        username=f"executor{unique}",
        hashed_password="x",
    )
    db.add(user)
    await db.flush()
